...
func resourceSecretsRead(d *schema.ResourceData, m interface{}) error {
	...
	decryptedSecrets, err := fetchExistingSecrets(sopsConfigPath, directoryPath, env, app)
	if err != nil {
...
func resourceSecretsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	existingSecrets, err := fetchExistingSecrets(sopsConfigPath, directoryPath, env, app)
	if err != nil {
		return err
	}
